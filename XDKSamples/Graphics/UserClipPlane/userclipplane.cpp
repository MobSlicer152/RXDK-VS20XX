...
XBHELP_CALLOUT g_HelpCallouts[] = 
{
    ...
    { XBHELP_B_BUTTON,     XBHELP_PLACEMENT_1, L"Next plane" },
    { XBHELP_LEFTSTICK,    XBHELP_PLACEMENT_2, L"Rotate\nplane X/Z" },
    { XBHELP_RIGHTSTICK,   XBHELP_PLACEMENT_2, L"Rotate plane Y\nand move" },
};
...
HRESULT CXBoxSample::InitVertexShader()
{
    ...
    XBUtil_CreateVertexShader( "shaders\\UserClipPlane.xvu", dwDecl, &m_dwVertexShader );
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_Model.Create( "Models\\Teapot.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
...
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::DrawPlanes()
{
    struct PLANE_VERTEX
    {
        D3DXVECTOR3 p;
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"UserClipPlane" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        swprintf( strBuffer, L"Plane %d of %d (%s)", m_dwCurrentPlane + 1, 
                                                     NUM_PLANES, 
