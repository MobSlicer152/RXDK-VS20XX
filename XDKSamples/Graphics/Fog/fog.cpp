...
HRESULT CXBoxSample::Initialize()
{
    ...
    m_pFloorTexture = m_xprResource.GetTexture( "SeaFloor" );
    ...
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\Fog.xvu",
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    // Toggle fixed function vs. vertex shader
    if( m_DefaultGamepad.bPressedAnalogButtons[XINPUT_GAMEPAD_A] )
        m_bUseVertexShader = !m_bUseVertexShader;

    // Adjust fog parameters
    if( m_dwFogMode == D3DFOG_EXP || m_dwFogMode == D3DFOG_EXP2 )
    {
...
HRESULT CXBoxSample::SetupVertexShader()
{
    ...
    // Set constants for computing fog values in the vertex shader
    if( m_dwFogMode == D3DFOG_NONE )
    {
    ...
    // Note: we set the fog table mode to LINEAR, EXP, or EXP2, which means
    // the vertex shader must output oFog.x in view space coordinates. The
    // hardware will take that value and perform the fog calculations on it 
    // (using the fog table lookup).
    //
    // Alternatively, if we set the fog table mode to D3DFOG_NONE above, we
    // would be telling the hardware that we want to use the value output from
    // the vertex shader as is, without any table lookup. In this case, the
    // vertex shader must output oFog.x in a range from 0.0 to 1.0.
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"Fog" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        WCHAR strBuffer[80];

        m_Font.DrawText(  64, 75, 0xffffffff, L"Pipeline:" );
        m_Font.DrawText( 200, 75, 0xffffff00, m_bUseVertexShader ? L"Vertex shader": L"Fixed-function" );
        
        m_Font.DrawText( 64, 100, 0xffffffff, L"Fog mode:" );
        if( m_dwFogMode == D3DFOG_NONE )
            m_Font.DrawText( 200, 100, 0xffffff00, L"D3DFOG_NONE"  );
        else if( m_dwFogMode == D3DFOG_LINEAR )
            m_Font.DrawText( 200, 100, 0xffffff00, L"D3DFOG_LINEAR" );
        else if( m_dwFogMode == D3DFOG_EXP )
            m_Font.DrawText( 200, 100, 0xffffff00, L"D3DFOG_EXP" );
        else if( m_dwFogMode == D3DFOG_EXP2 )
            m_Font.DrawText( 200, 100, 0xffffff00, L"D3DFOG_EXP2" );

        if( m_dwFogMode == D3DFOG_EXP || m_dwFogMode == D3DFOG_EXP2 )
        {
            swprintf( strBuffer, L"%0.3f", m_fFogDensity );
            m_Font.DrawText(  64, 125, 0xffffffff, L"Density:" );
            m_Font.DrawText( 200, 125, 0xffffff00, strBuffer );
        }
        else
        {
            swprintf( strBuffer, L"%0.2f", m_fFogStartValue );
            m_Font.DrawText(  64, 125, 0xffffffff, L"Start:" );
            m_Font.DrawText( 200, 125, 0xffffff00, strBuffer  );

            swprintf( strBuffer, L"%0.2f", m_fFogEndValue );
            m_Font.DrawText(  64, 150, 0xffffffff, L"End:" );
            m_Font.DrawText( 200, 150, 0xffffff00, strBuffer  );
        }
