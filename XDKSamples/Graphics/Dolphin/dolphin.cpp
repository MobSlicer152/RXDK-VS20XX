...
struct BUBBLE
{
    FLOAT       fAge, fLifespan;
    FLOAT       fAngle, fSpread, fWobble;
    D3DXVECTOR3 vPos;
    D3DXVECTOR3 vPos0;
};
...
class CXBoxSample : public CXBApplication
{
    ...
    D3DXVECTOR3             m_vDolphinPos;
    D3DXVECTOR3             m_vDolphinDir;
    D3DXMATRIX              m_matDolphin;
    D3DXMATRIX              m_matDolphinWV;
    D3DXMATRIX              m_matDolphinWVP;
    DWORD                   m_dwAmbient;
    D3DXVECTOR4             m_fFog;
    ...
    D3DXVECTOR3*            m_pBubbleVertices;
    DWORD                   m_dwNumBubblesToRender;
            ...
            :CXBApplication()
{
    ...
    m_pBubbleVertices        = new D3DXVECTOR3[m_dwNumBubbles];
    m_dwNumBubblesToRender   = 0;

    for( DWORD i=0; i<m_dwNumBubbles; i++ )
    {
        m_pBubbles[i].fLifespan = 5.0f + ((FLOAT)rand()/(FLOAT)RAND_MAX) * 2.0f;
        m_pBubbles[i].fAge      = (i*m_pBubbles[i].fLifespan)/m_dwNumBubbles;
        m_pBubbles[i].fAngle    = ((FLOAT)rand()/(FLOAT)RAND_MAX) * 2.0f * D3DX_PI;
        m_pBubbles[i].fSpread   = ((FLOAT)rand()/(FLOAT)RAND_MAX) * 1.0f;
        m_pBubbles[i].fWobble   = ((FLOAT)rand()/(FLOAT)RAND_MAX) * 2.0f * D3DX_PI - D3DX_PI;

        FLOAT y = m_pBubbles[i].fAge;
        FLOAT r = m_pBubbles[i].fSpread * sqrtf( y / 10.0f );
        FLOAT x = r*sinf( m_pBubbles[i].fAngle );
        FLOAT z = r*cosf( m_pBubbles[i].fAngle );

        x += 0.1f*sqrtf( y / 4.0f ) * sinf( 1 * y * m_pBubbles[i].fWobble );
        z += 0.1f*sqrtf( y / 4.0f ) * cosf( 1 * y * m_pBubbles[i].fWobble );
...
// Desc: Creates the pixel shaders for the both rendering the dolphin and
//       rendering the seafloor
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::CreatePixelShader()
{
    ...
    //------------- Stage 0 -------------
    // Build factor from caustics and lighting (R0 = T1*V0, R1=C0)
    psd.PSRGBInputs[0]    = PS_COMBINERINPUTS( PS_REGISTER_T1  | PS_CHANNEL_RGB,
                                               PS_REGISTER_V0  | PS_CHANNEL_RGB,
                                               PS_REGISTER_C0  | PS_CHANNEL_RGB,
                                               PS_REGISTER_ONE | PS_CHANNEL_RGB );
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_DolphinObject1.Create( "Models\\Dolphin1.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    if( FAILED( m_DolphinObject2.Create( "Models\\Dolphin2.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    if( FAILED( m_DolphinObject3.Create( "Models\\Dolphin3.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    hr = XBUtil_CreateVertexShader( "Shaders\\Dolphin.xvu",
                                    dwDolphinVertexDecl, &m_dwDolphinVertexShader );
    if( FAILED(hr) )
        return hr;

    // Load the file-based mesh object for the seafloor
    if( FAILED( m_SeaFloorObject.Create( "Models\\SeaFloor.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if( FAILED( hr = XBUtil_CreateVertexShader( "Shaders\\SeaFloor.xvu",
    ...
    CXBMesh xbgPath;
    if( FAILED( xbgPath.Create( "Models\\Path.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if( FAILED( m_xprResources.Create( "Resource.xpr", resource_NUM_RESOURCES ) ) )
        return E_FAIL;
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    XBUtil_GetSplinePoint( m_DolphinSpline, m_dwNumDolphinSplinePts, u, 
                           &m_vDolphinPos, &m_vDolphinDir );

    // Scale and orient the dolphin model
    D3DXMATRIX matScale, matRotateY;
    D3DXMatrixScaling( &matScale, 0.01f, 0.01f, 0.01f );
    D3DXMatrixRotationY( &matRotateY, D3DX_PI/2 );
    D3DXMatrixMultiply( &m_matDolphin, &matScale, &matRotateY );
    ...
    D3DXMatrixMultiply( &m_matDolphin, &m_matDolphin, &matKickRotate );
    m_vDolphinPos.y += sinf(fKickFreq)/2;
    ...
    D3DXMATRIX matPathLookAt;
    const D3DXVECTOR3 vLookAt( 0.0f, 0.0f, 0.0f );
    const D3DXVECTOR3 vUp    ( 0.0f, 1.0f, 0.0f );
    D3DXMatrixLookAtLH( &matPathLookAt, &vLookAt, &m_vDolphinDir, &vUp );

    D3DXMATRIX matDolphinRotate;
    D3DXMatrixInverse( &matDolphinRotate, NULL, &matPathLookAt );

    D3DXMATRIX matDolphinTrans;
    D3DXMatrixTranslation( &matDolphinTrans, m_vDolphinPos.x, m_vDolphinPos.y, m_vDolphinPos.z );
    D3DXMatrixMultiply( &m_matDolphin, &m_matDolphin, &matDolphinRotate );
    D3DXMatrixMultiply( &m_matDolphin, &m_matDolphin, &matDolphinTrans );

    // Position the bubble emitter position to correspond to the dolphin's blowhole.
    D3DXVECTOR3 vBlowHole( -150.0f, 40.0f, 0.0f );
    D3DXVec3TransformCoord( &m_vBubbleEmitterPos, &vBlowHole, &m_matDolphin );
    ...
    m_dwAmbient = (DWORD)(255.0f*0.5f*fDepthFactor);
    m_dwAmbient = (m_dwAmbient<<16L) | (m_dwAmbient<<8L) | (m_dwAmbient<<0L);

    m_fFog = fFog;

    // Build matrix sets (and transpose them) for the vertex shaders
    D3DXMatrixMultiply( &m_matDolphinWV, &m_matDolphin, &m_matView );
    D3DXMatrixMultiply( &m_matDolphinWVP, &m_matDolphinWV, &m_matProj );
    D3DXMatrixTranspose( &m_matDolphinWV,  &m_matDolphinWV );
    D3DXMatrixTranspose( &m_matDolphinWVP,  &m_matDolphinWVP );
    ...
    m_pd3dDevice->SetVertexShaderConstant( 20, &m_matDolphinWV,   4 ); // Dolphin WV matrix set
    m_pd3dDevice->SetVertexShaderConstant( 24, &m_matDolphinWVP,  4 ); // Dolphin WVP matrix set
    ...
    m_pd3dDevice->SetVertexShaderConstant( 43, &m_fFog,           1 ); // Fog factors
...
//-----------------------------------------------------------------------------
// Name: UpdateBubbles()
// Desc: Updates the particle system that controls the bubbles
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::UpdateBubbles( FLOAT fAddBubbles )
{
    ...
    m_dwNumBubblesToRender = 0L;

    for( DWORD i=0; i<m_dwNumBubbles; i++ )
    {
        BUBBLE* pBubble = &m_pBubbles[i];

        if( pBubble->fAge > pBubble->fLifespan )
        {
            if( fBubblesToAdd > 1.0f )
            {
                fBubblesToAdd -= 1.0f;

                pBubble->fAge      = pBubble->fAge - pBubble->fLifespan;
                pBubble->fLifespan = 3.0f + ((FLOAT)rand()/(FLOAT)RAND_MAX) * 1.0f;
                pBubble->vPos0     = m_vBubbleEmitterPos;
            }
        }
        else
            pBubble->fAge += m_fElapsedAppTime;

        FLOAT y = 1.2f*pBubble->fAge;
        FLOAT r = pBubble->fSpread * sqrtf( y / 10.0f );
        FLOAT x = r*sinf( pBubble->fAngle );
        FLOAT z = r*cosf( pBubble->fAngle );

        x += 0.1f*sqrtf( y / 4.0f ) * sinf( 1 * y * pBubble->fWobble );
        z += 0.1f*sqrtf( y / 4.0f ) * cosf( 1 * y * pBubble->fWobble );

        pBubble->vPos.x = pBubble->vPos0.x + x;
        pBubble->vPos.y = pBubble->vPos0.y + y;
        pBubble->vPos.z = pBubble->vPos0.z + z;

        if( pBubble->fAge < pBubble->fLifespan )
        {
            m_pBubbleVertices[m_dwNumBubblesToRender++] = pBubble->vPos;
        }
    }
...
//-----------------------------------------------------------------------------
// Name: RenderBubbles()
// Desc: Renders the bubbles particle system
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::RenderBubbles()
{
    if( 0 == m_dwNumBubblesToRender )
        return S_OK;
    ...
    m_pd3dDevice->DrawVerticesUP( D3DPT_POINTLIST, m_dwNumBubblesToRender,
                                  m_pBubbleVertices, sizeof(D3DXVECTOR3) );
...
HRESULT CXBoxSample::Render()
{
    // Render the dolphin's shadow
    {
        ...
        static D3DVIEWPORT8 vpShadow = { 0, 0, SHADOW_SIZE, SHADOW_SIZE, 0.0f, 1.0f };
        ...
        D3DXVECTOR3 vShadowEyePt = m_vDolphinPos + 10 * D3DXVECTOR3( sinf(g_fLightAngle), cosf(g_fLightAngle), 0.0f );
        D3DXMATRIX  matShadowView;
        D3DXMATRIX  matShadowProj;
        D3DXMatrixLookAtLH( &matShadowView, &vShadowEyePt, &m_vDolphinPos, &m_vDolphinDir );
        D3DXMatrixOrthoLH( &matShadowProj, 3.5f, 7.0f, 1.0f, 200.0f );
                   
        D3DXMATRIX matDolphinShadowWV, matDolphinShadowWVP;
        D3DXMatrixMultiply( &matDolphinShadowWV,  &m_matDolphin,         &matShadowView );
        ...
        m_pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, g_dwShadowColor );
        ...
        static D3DVIEWPORT8 vpBackBuffer = { 0, 0, 640, 480, 0.0f, 1.0f };
    ...
    // current caustic texture and the dolphin's shadow.
    ...
    m_pd3dDevice->SetRenderState( D3DRS_PSCONSTANT0_0, m_dwAmbient );
    ...
    m_Font.DrawText( 48, 36, 0xffffffff,  L"Dolphin" );
