...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_Room.Create( "Models\\VolFogTerrain.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    // Load the main file object
    if( FAILED( m_FogVolume.Create( "Models\\VolFogFog.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if ( FAILED( XBUtil_CreateVertexShader( "Shaders\\DepthShader.xvu", vdecl, &m_dwFogVertexShader ) ) )
        return E_FAIL;
...
HRESULT CXBoxSample::Render()
{
    ...
    m_FogVolume.Render( XBMESH_NOFVF | XBMESH_NOTEXTURES );
    ...
    m_Room.Render( XBMESH_NOFVF | XBMESH_NOTEXTURES );
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"VolumeFog" );
