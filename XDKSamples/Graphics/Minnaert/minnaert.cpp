...
BOOL CXBWomanMesh::RenderCallback( DWORD dwSubset, XBMESH_SUBSET* pSubset, DWORD dwFlags )
{
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CreatePixelShader1( DWORD* pdwPixelShader )
{
    ...
    if( FAILED( D3DDevice::CreatePixelShader( &psd, pdwPixelShader ) ) )
        return E_FAIL;
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CreatePixelShader2( DWORD* pdwPixelShader )
{
    D3DPIXELSHADERDEF psd;
    ZeroMemory(&psd, sizeof(psd));
...
HRESULT CXBoxSample::Initialize()
{
    ...
    // Load geometry
    if( FAILED( m_LightObject.Create( "Models\\DirLight.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    m_vLightCenter = D3DXVECTOR3( 0, 0, 0 );
    m_fLightRadius = 0.5f;

    if( FAILED( m_WomanObject.Create( "Models\\Woman.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\BRDF.xvu",
                                           dwWomanVertexDecl,
                                           &m_dwWomanVertexShader ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    if( FAILED( CreatePixelShader1( &m_dwWomanPixelShader1 ) ) )
        return E_FAIL;
    if( FAILED( CreatePixelShader2( &m_dwWomanPixelShader2 ) ) )
        return E_FAIL;
    ...
    if( FAILED( XBUtil_CreateNormalizationCubeMap( 256, &m_pNormalizationCubemap ) ) )
        return E_FAIL;
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    // Update the light orientation
    {
        ...
        D3DXVec3TransformNormal( &m_LightDir, &vZ, &matRotateLight );
    }
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"Minnaert" );
        ...
        if( m_bUseCustomLighting )
        {
            switch( m_LightingEffect )
            {
                case MINNAERT: m_Font.DrawText( 64, 75, 0xffffff00, L"Minnaert lighting" ); break;
                case SATIN:    m_Font.DrawText( 64, 75, 0xffffff00, L"Satin lighting" );    break;
                case VELVET:   m_Font.DrawText( 64, 75, 0xffffff00, L"Velvet lighting" );   break;
            }
        }
        else
            m_Font.DrawText(  64, 75, 0xffffff00, L"Normal lighting" );
