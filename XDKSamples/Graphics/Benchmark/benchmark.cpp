...
CXBoxSample::CXBoxSample()
{
    ...
    // Increase the default push buffer size
    Direct3D_SetPushBufferSize( 4*1024*1024, 1024*1024 );
...
{
    // Set inside material
    ZeroMemory( &m_mtrlInside,  sizeof(D3DMATERIAL8) );
    m_mtrlInside.Diffuse.r  = 1.0f;
    m_mtrlInside.Diffuse.g  = 0.0f;
    m_mtrlInside.Diffuse.b  = 0.0f;
    m_mtrlInside.Diffuse.a  = 1.0f;
    m_mtrlInside.Ambient.r  = 1.0f;
    m_mtrlInside.Ambient.g  = 0.0f;
    m_mtrlInside.Ambient.b  = 0.0f;
    m_mtrlInside.Ambient.a  = 1.0f;

    // Set outside materials
...
HRESULT CXBoxSample::Render()
{
    ...
    for( i = 0; i < g_dwRibbonCount; i++ )
    {
        ...
        // Draw outside ribbon
        m_pd3dDevice->DrawIndexedPrimitive( dwPrimType, 0, m_dwNumVertices, 0, dwTriCount );
    }
    ...
    // All ribbons are using the same inside material
...
VOID CXBoxSample::DisplayInfo()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"Benchmark" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        if( m_dwDisplayInfo == DISPLAY_INSTANTANEOUSSTATS )
        {
            m_Font.DrawText( 320, 60, 0xffffffff, L"Instantaneous Stats", XBFONT_CENTER_X );

            swprintf( str, L"%d x %d", 640, 480 );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Size:" );
            m_Font.DrawText( 384, iY, 0xffffff00, str, XBFONT_RIGHT );
            iY += 20;

            m_Font.DrawText(  64, iY, 0xffffffff, L"Z-Depth:" );
            switch( m_d3dpp.AutoDepthStencilFormat )
            {
                case D3DFMT_D16:
                case D3DFMT_LIN_D16:
                    m_Font.DrawText( 384, iY, 0xffffff00, L"16-bit", XBFONT_RIGHT );
                    break;
                case D3DFMT_D24S8:
                case D3DFMT_LIN_D24S8:
                    m_Font.DrawText( 384, iY, 0xffffff00, L"24-bit w/stencil", XBFONT_RIGHT );
                    break;
                default:
                    m_Font.DrawText( 384, iY, 0xffffff00, L"Unknown", XBFONT_RIGHT );
                    break;
            }
            iY += 40;

            swprintf( str, L"%0.3lf", m_fTPS * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Triangles per sec:\n" );
            m_Font.DrawText( 384, iY, 0xffffff00, str, XBFONT_RIGHT );
            m_Font.DrawText( 384, iY, 0xffffff00, L" MTris/s" );
            iY += 20;
            swprintf( str, L"%0.3lf", m_fSPS * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Vertices per sec:\n" );
            m_Font.DrawText( 384, iY, 0xffffff00, str, XBFONT_RIGHT );
            m_Font.DrawText( 384, iY, 0xffffff00, L" MVerts/s" );
            iY += 40;

            swprintf( str, L"%0.3lf", m_fIPS * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Index traffic:\n" );
            m_Font.DrawText( 384, iY, 0xffffff00, str, XBFONT_RIGHT );
            m_Font.DrawText( 384, iY, 0xffffff00, L" MBytes/s" );
            iY += 20;
            swprintf( str, L"%0.3lf", m_fVPS * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Vertex traffic:\n" );
            m_Font.DrawText( 384, iY, 0xffffff00, str, XBFONT_RIGHT );
            m_Font.DrawText( 384, iY, 0xffffff00, L" MBytes/s" );
            iY += 20;
        }
        ...
        {
            m_Font.DrawText( 320, 60, 0xffffffff, L"Rendering Options", XBFONT_CENTER_X );

            m_Font.DrawText(  64, iY, 0xffffffff, L"Primitive Type:" );
            m_Font.DrawText( 274, iY, 0xffffff00, m_bUseTriList ? L"Triangle List" : L"Triangle Strip" );
            iY += 20;
            m_Font.DrawText(  64, iY, 0xffffffff, L"Fillmode:" );
            if( m_wFillMode == D3DFILL_POINT )     m_Font.DrawText( 274, iY, 0xffffff00, L"Point" );
            if( m_wFillMode == D3DFILL_WIREFRAME ) m_Font.DrawText( 274, iY, 0xffffff00, L"Wireframe" );
            if( m_wFillMode == D3DFILL_SOLID )     m_Font.DrawText( 274, iY, 0xffffff00, L"Solid" );
            iY += 20;
            swprintf( str, L"%d", m_dwNumTextures );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Num Textures:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            swprintf( str, L"%d", m_dwNumDirectionalLights );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Num Dir Lights:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            swprintf( str, L"%d", m_dwNumPointLights );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Num Point Lights:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            swprintf( str, L"%d", m_dwNumSpotLights );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Num Spot Lights:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            iY += 10;
            ...
            m_Font.DrawText(  64, iY, 0xffffffff, L"Triangles per frame:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            swprintf( str, L"%d indices", 2 * g_dwRibbonCount * m_dwNumIndices );
            m_Font.DrawText(  64, iY, 0xffffffff, L"Indices per frame:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            swprintf( str, L"%d bytes", m_dwFVFSize );
            m_Font.DrawText(  64, iY, 0xffffffff, L"FVFSize:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;
            iY += 10;
    
            swprintf( str, L"%0.3f Mtps", m_fCumulativeTriPerSec * 0.000001f / m_dwFrame );
            m_Font.DrawText(  64, iY, 0xffffffff, L"AvgTriPerSec:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;

            swprintf( str, L"%0.3f Mtps", m_fMaxTriPerSec * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"MaxTriPerSec:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
            iY += 20;

            swprintf( str, L"%0.3f Mtps", m_fMinTriPerSec * 0.000001f );
            m_Font.DrawText(  64, iY, 0xffffffff, L"MinTriPerSec:" );
            m_Font.DrawText( 274, iY, 0xffffff00, str );
        }
    }
