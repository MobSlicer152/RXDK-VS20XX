...
//
// Desc: Example showing how get correct alpha blending without sorting.
//       Uses depth peeling to separate the alpha into layers.
//
...
// For the furthest layer of alpha, blend then write z values in separate passes
// so that nearer alpha values won't prevent further alpha values from being
...
public:
    ...
    {
        D3DDevice::SetPixelShaderConstant( 0, &pSubset->mtrl.Diffuse, 1 );
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_Room.Create( "Models\\Room2.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    // Load the main file object
    if( FAILED( m_AlphaVolume.Create( "Models\\alphaobjs.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if ( FAILED( XBUtil_CreateVertexShader( "Shaders\\AlphaShader.xvu", vdecl, &m_dwAlphaVertexShader ) ) )
        return E_FAIL;
...
HRESULT CXBoxSample::Render()
{
    ...
    if( m_bUseDepthPeeling )
    {
        ...
        for( int i = 0; i < m_iNumLayers-1; i++ )
        {
            ...
            m_AlphaVolume.Render( XBMESH_NOFVF | XBMESH_NOTEXTURES | XBMESH_NOMATERIALS );
        }
    ...
    else
    {
        ...
        // Draw any alpha blended parts of the scene.
        m_AlphaVolume.Render( XBMESH_NOTEXTURES | XBMESH_NOMATERIALS );
    }    
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"NoSortAlphaBlend" );
        ...
        if( m_bUseDepthPeeling )
        {
            ...
            {
                WCHAR buf[80];
                swprintf( buf, L"%d", m_iDrawLayer+1 );
                m_Font.DrawText( 64, 70, 0xffffffff, L"Layer = " );
                m_Font.DrawText( 0xffffff00, buf );
            }
            else
            {
                WCHAR buf[80];
                swprintf( buf, L"%d", m_iNumLayers );
                m_Font.DrawText( 64, 70, 0xffffffff, L"NumLayers = " );
                m_Font.DrawText( 0xffffff00, buf );
            }

            if( m_bUseDotZW )
            {
                m_Font.DrawText( 280, 70, 0xffffff00, L"Depth Replace" );
            }
        }
        else
        {
            m_Font.DrawText( 64, 70, 0xffffff00, L"Incorrect Blending" );
        }
    }
