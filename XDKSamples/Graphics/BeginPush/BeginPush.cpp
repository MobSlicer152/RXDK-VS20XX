            ...
            :CXBApplication()
{
    ...
    for( WORD i=0; i<NUM_STREAKS; i++ )
    {
        ...
        for( DWORD j=0; j<TRAIL_SIZE; j++ )
        {
            const D3DXVECTOR3 vNowhere( 10000.0f, 10000.0f, 10000.0f );
            g_pStreaks[i].vLastPositions[j] = vNowhere;
        }

        g_pStreaks[i].vDir = D3DXVECTOR3( rnd() - rnd(), rnd() - rnd(), rnd() - rnd() );
        D3DXVec3Normalize( &g_pStreaks[i].vDir, &g_pStreaks[i].vDir );
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    // Update the flock at 60Hz, regardless of frame rate. The trigger variable
...
HRESULT CXBoxSample::Render()
{
    ...
    // Set the render states for using point sprites. Note that point sprites
        ...
        case USEBEGINPUSH: // Draw the streaks with the BeginPush() API
        {
            ...
            for( WORD i=0; i<NUM_STREAKS; i++ )
            {
                ...
                *pPush++ = D3DPT_POINTLIST;
                ...
                // of 2047 DWORDs can be specified in an INLINE_ARRAY section.
    ...
    // Capture the stop time, and calculate the elapsed CPU-time it took to
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"BeginPush" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        // Show the perf-timing results
        WCHAR str[80];
        m_Font.DrawText( 64, 80, 0xffffffff, L"Using draw method:" );
        switch( m_DrawMethod )
        {
            case USEDRAWPRIMUP: m_Font.DrawText( 270, 80, 0xffffff00, L"DrawPrimitiveUP()" ); break;
            case USEBEGINPUSH:  m_Font.DrawText( 270, 80, 0xffffff00, L"BeginPush()" ); break;
            case USEBEGIN:      m_Font.DrawText( 270, 80, 0xffffff00, L"Begin()" ); break;
        }
        swprintf( str, L"%.3f ms", 1000*m_fElapsedTime );
        m_Font.DrawText(  64, 105, 0xffffffff, L"Draw method took:" );
        m_Font.DrawText( 270, 105, 0xffffff00, str );
...
//-----------------------------------------------------------------------------
// Name: UpdateFlock()
// Desc: Update position of each streak in flock
//-----------------------------------------------------------------------------
VOID UpdateFlock()
{
    FLOAT fDist;
    ...
    for( i = 0; i < NUM_STREAKS; i++ )
    {
        ...
        {
            D3DXVECTOR3 vDistance = g_pStreaks[i].vPos - g_pStreaks[j].vPos;
            fDist = D3DXVec3LengthSq( &vDistance );
    ...
    for( i = 0; i < NUM_STREAKS; i++ )
    {
        for( WORD j = i + 1; j < NUM_STREAKS; j++ )
        {
            ...
            if( g_pStreaks[i].afDist[j] > 0.0f )
            {
                ...
                D3DXVECTOR3 vDistance = g_pStreaks[i].vPos - g_pStreaks[j].vPos;
                D3DXVec3Normalize( &vDiff, &vDistance );
    ...
    for( i=0; i<NUM_STREAKS; i++ )
    {
        ...
        D3DXVECTOR3 vGoal = g_vGoal - g_pStreaks[i].vPos;
    ...
    for( WORD i=0; i<NUM_STREAKS; i++)
    {
        ...
        // Insert a deliberate CPU stall to emulate an expensize physics calculation.
        // The big difference between the rendering API is the CPU use, so we need
        // a CPU bound app to show their difference
        for( DWORD j=0; j<1000; j++ )
        {
            // Note: sin^2 + cos^2 = 1, so this code purposely does nothing!
            g_pStreaks[i].vPos *= sinf( (FLOAT)j ) * sinf( (FLOAT)j ) + cosf( (FLOAT)j ) * cosf( (FLOAT)j );
        }
    }
