...
class CXBoxSample : public CXBApplication
{
    ...
    FLOAT                   m_fWidth;
    FLOAT                   m_fHeight;
    LONG                    m_nMipLevel;            // which texture mipmap level to display
...
HRESULT CXBoxSample::Render()
{
        ...
        switch( m_dwTypes[m_nCurrent] )
        {
            case D3DCOMMON_TYPE_TEXTURE:
                ...
                if( g_TextureFormats[x].id != 0 )
                {
                    ...
                    lLevels  = (*(pRes+3) & D3DFORMAT_MIPMAP_MASK) >> D3DFORMAT_MIPMAP_SHIFT;
                    ...
                    {
                        if( m_nMipLevel == -1 )
                            swprintf( strLevels, L"%d levels", lLevels );
                        else
                        {
                            if( m_nMipLevel >= lLevels )
                                m_nMipLevel = lLevels - 1; // constrain choice of miplevel to appropriate range
                            swprintf( strLevels, L"level %d of %d levels", m_nMipLevel, lLevels );
                        }
                    }
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"XPRViewer" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );
        m_Font.DrawText(  64, 80, 0xff8080ff, strDescription );
        m_Font.End();
    }
