...
HRESULT CXBoxSample::Initialize()
{
    ...
    // Load our geometry
    if( FAILED( m_SkyBoxObject.Create( "Models\\SkyBox.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    if( FAILED( m_FlameObject.Create( "Models\\Flame.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    m_pGroundTexture        = m_xprResource.GetTexture( "FireGround" );
    m_pFireBaseTexture      = m_xprResource.GetTexture( "FlameColor" );
    m_pFireNoiseTexture     = m_xprResource.GetTexture( "FireNoise" );
    m_pFireNoiseMaskTexture = m_xprResource.GetTexture( "NoiseAlpha" );
    ...
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\Fire.xvu",
                                           dwFireVertexDecl, &m_dwFireVertexShader ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    
    if( FAILED( XBUtil_CreatePixelShader( "Shaders\\Fire.xpu",
                                          &m_dwFirePixelShader ) ) )
        return XBAPPERR_MEDIANOTFOUND;
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    // Move the camera
    static D3DXVECTOR3 vCameraPos( 0.0f, 2.0f, -20.0f );
    const D3DXVECTOR3 vX( 1.0f, 0.0f, 0.0f );
    const D3DXVECTOR3 vY( 0.0f, 1.0f, 0.0f );
    const D3DXVECTOR3 vZ( 0.0f, 0.0f, 1.0f );
    D3DXVECTOR3 vForward;
    D3DXVec3TransformCoord( &vForward, &vZ, &matRotateY );
    D3DXVECTOR3 vRight;
    D3DXVec3TransformCoord( &vRight, &vX, &matRotateY );
    ...
    D3DXVECTOR3 vLookat;
    D3DXVec3TransformCoord( &vLookat,  &vZ, &matRotate );
    D3DXVec3Add( &vLookat, &vLookat, &vCameraPos );
    ...
    vLookat = D3DXVECTOR3( -vCameraPos.x, 0.0f, -vCameraPos.z );
    D3DXMATRIX matBillboard;
    D3DXMatrixLookAtLH( &matBillboard, &vZero,
                                       &vLookat,
                                       &vY );
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"Fire" );
