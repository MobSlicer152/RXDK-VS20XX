...
#define D3DFVF_PRELITVERTEX (D3DFVF_XYZ|D3DFVF_DIFFUSE)
...
CXBFur::CXBFur()
{
    ...
    m_fuzzCenter.colorBase = D3DXCOLOR( 1.0f, 1.0f, 1.0f, 1.0f );
    m_fuzzCenter.colorTip  = D3DXCOLOR( 1.0f, 1.0f, 1.0f, 0.0f );
    m_fuzzCenter.vel.y     = 1.0f;
    ...
    m_dwSliceXSize = 0;
    m_dwSliceZSize = 0;
    for( DWORD i=0; i<XBFUR_MAXSLICE*2-1; i++ )
...
void CXBFur::InitFuzz( DWORD nfuzz, DWORD nfuzzlib )
{
    ...
    if( m_dwNumFuzz != nfuzz )
    {
        SAFE_DELETE( m_pFuzz );
        m_pFuzz     = new FuzzInst[nfuzz];
        m_dwNumFuzz = nfuzz;
    }
    ...
    for( i=0; i<m_dwNumFuzzLib; i++ )
    {
        ...
        m_pFuzzLib[i].vel.z = (m_fuzzCenter.vel.z + m_fuzzRandom.vel.z*(2*frand(1.0f)-1.0f))*g_fOneInch;

        m_pFuzzLib[i].acc.x = (m_fuzzCenter.acc.x + m_fuzzRandom.acc.x*(2*frand(1.0f)-1.0f))*g_fOneInch;
        m_pFuzzLib[i].acc.y = (m_fuzzCenter.acc.y + m_fuzzRandom.acc.y*(2*frand(1.0f)-1.0f))*g_fOneInch;
        m_pFuzzLib[i].acc.z = (m_fuzzCenter.acc.z + m_fuzzRandom.acc.z*(2*frand(1.0f)-1.0f))*g_fOneInch;

        m_pFuzzLib[i].colorBase = m_fuzzCenter.colorBase + (2*frand(1.0f)-1.0f)*m_fuzzRandom.colorBase;
        m_pFuzzLib[i].colorTip  = m_fuzzCenter.colorTip  + (2*frand(1.0f)-1.0f)*m_fuzzRandom.colorTip;

        FLOAT y = m_pFuzzLib[i].vel.y + 0.5f*m_pFuzzLib[i].acc.y;
        if( y > m_fYSize )
            m_fYSize = y;
    }
...
void CXBFur::GenSlices(DWORD nslices, DWORD slicexsize, DWORD slicezsize)
{
    ...
    if( m_dwSliceXSize!=slicexsize || m_dwSliceZSize!=slicezsize || !bFormatOK )
    {
        m_dwNumSlices = 0;
        m_dwNumSlicesLOD = 0;
    ...
    if( m_dwNumSlices != nslices )
    {
        ...
        for( i=0; i<nslices+nLOD; i++ )
            if( !m_apSliceTexture[i] )
                g_pd3dDevice->CreateTexture( slicexsize, slicezsize, 0, 0, D3DFMT_A8R8G8B8,
                                             0, &m_apSliceTexture[i] );
    ...
    D3DSurface* pSavedBackBuffer;
    D3DSurface* pSavedZBuffer;
    D3DMATRIX matWorldSaved, matViewSaved, matProjectionSaved;
    g_pd3dDevice->GetRenderTarget(&pSavedBackBuffer);
    g_pd3dDevice->GetDepthStencilSurface(&pSavedZBuffer);
    g_pd3dDevice->GetTransform( D3DTS_WORLD, &matWorldSaved );
    g_pd3dDevice->GetTransform( D3DTS_VIEW, &matViewSaved );
    g_pd3dDevice->GetTransform( D3DTS_PROJECTION, &matProjectionSaved );
        
    // make a new depth buffer
    D3DSurface* pZBuffer = NULL;
    ...
    g_pd3dDevice->SetVertexShader(D3DFVF_PRELITVERTEX);
    g_pd3dDevice->SetStreamSource(0, pVB, sizeof(PRELITVERTEX));
    ...
    pVB->Release();
    pZBuffer->Release();
    g_pd3dDevice->SetRenderTarget( pSavedBackBuffer, pSavedZBuffer );
    pSavedBackBuffer->Release();
    pSavedZBuffer->Release();
    g_pd3dDevice->SetTransform( D3DTS_WORLD,      &matWorldSaved );
    g_pd3dDevice->SetTransform( D3DTS_VIEW,       &matViewSaved );
    g_pd3dDevice->SetTransform( D3DTS_PROJECTION, &matProjectionSaved );
...
//-----------------------------------------------------------------------------
void CXBFur::GenFin( DWORD finWidth, DWORD finHeight,
                     FLOAT fFinXFraction, FLOAT fFinZFraction )
{
    ...
    if( m_pFinTexture == NULL )
        g_pd3dDevice->CreateTexture( finWidth, finHeight, 0, 0, D3DFMT_A8R8G8B8,
                                     0, &m_pFinTexture );
...
//-----------------------------------------------------------------------------
void CXBFur::GetLinesVertexBuffer( D3DVertexBuffer** ppVB )
{
    D3DVertexBuffer* pVB;
    UINT numVertices = m_dwNumFuzz*m_dwNumSegments*2;
    g_pd3dDevice->CreateVertexBuffer(numVertices*sizeof(PRELITVERTEX), 0, D3DFVF_PRELITVERTEX, 0, &pVB);
    assert(pVB!=NULL);
    DWORD i, j, vidx, lidx;
    PRELITVERTEX *verts;
    float x0, y0, z0;
    ...
    pVB->Lock(0, numVertices*sizeof(PRELITVERTEX), (BYTE **)&verts, 0);
    vidx = 0;
    ...
    for(i=0; i<m_dwNumFuzz; i++)
    {
        ...
        dx = m_pFuzzLib[lidx].vel.x;
        dy = m_pFuzzLib[lidx].vel.y;
        dz = m_pFuzzLib[lidx].vel.z;
        ddx = m_pFuzzLib[lidx].acc.x;
        ddy = m_pFuzzLib[lidx].acc.y;
        ddz = m_pFuzzLib[lidx].acc.z;
        ...
        {
            verts[vidx].p.x = x0 + cp*dx + 0.5f*cp*cp*ddx;
            verts[vidx].p.y = y0 + cp*dy + 0.5f*cp*cp*ddy;
            verts[vidx].p.z = z0 + cp*dz + 0.5f*cp*cp*ddz;
...
void CXBFur::Save(char *fname, int flags)
{
    ...
    WriteFile(hFile, &m_fuzzCenter.vel, sizeof(D3DXVECTOR3), &dwNumBytesWritten, NULL);
    WriteFile(hFile, &m_fuzzRandom.vel, sizeof(D3DXVECTOR3), &dwNumBytesWritten, NULL);
    WriteFile(hFile, &m_fuzzCenter.acc, sizeof(D3DXVECTOR3), &dwNumBytesWritten, NULL);
    WriteFile(hFile, &m_fuzzRandom.acc, sizeof(D3DXVECTOR3), &dwNumBytesWritten, 0);
...
void CXBFur::Load(char *fname)
{
    ...
    ReadFile(hFile, &m_fuzzCenter.vel, sizeof(D3DXVECTOR3), &dwNumBytesRead, NULL);  // velocity center
    ReadFile(hFile, &m_fuzzRandom.vel, sizeof(D3DXVECTOR3), &dwNumBytesRead, NULL);  // velocity random
    ReadFile(hFile, &m_fuzzCenter.acc, sizeof(D3DXVECTOR3), &dwNumBytesRead, NULL); // acceleration center
    ReadFile(hFile, &m_fuzzRandom.acc, sizeof(D3DXVECTOR3), &dwNumBytesRead, NULL); // acceleration random
...
inline float Luminance(const D3DXCOLOR &c)
{
...
inline float MaxChannel(const D3DXCOLOR &c)
{
    if (c.r > c.g)
    {
        if (c.r > c.b)
...
inline D3DXCOLOR Lerp(const D3DXCOLOR &c1, const D3DXCOLOR &c2, float s)
{
    return c1 + s * (c2 - c1);
}
...
inline D3DXCOLOR Desaturate(const D3DXCOLOR &rgba)
{
    float alpha = rgba.a;
    if (alpha > 1.f)
        alpha = 1.f;
...
struct HairLighting 
{
    ...
    HRESULT Initialize( D3DMATERIAL8* pMaterial );
...
HRESULT HairLighting::Initialize( D3DMATERIAL8* pMaterial )
{
...
// Desc: Create a hair lighting lookup-table texture
//-----------------------------------------------------------------------------
                                 ...
                                 LPDIRECT3DTEXTURE8 pTexture )
{
    ...
    pTexture->LockRect( 0, &lockedRect, NULL, 0l );

    HairLighting lighting;
    lighting.Initialize( pMaterial );
...
HRESULT CXBFur::SetHairLightingMaterial( D3DMATERIAL8* pMaterial )
{
    ...
    {
        // Create a small 8x8 texture
        hr = D3DXCreateTexture( g_pd3dDevice, 8, 8, 1, 0, D3DFMT_A8R8G8B8,
                                0, &m_pHairLightingTexture );
        if( FAILED(hr) )
            goto e_Exit;
    }
...
//-----------------------------------------------------------------------------
// Name: SetLevelOfDetail()
// Desc: Choose level of detail 
//
...
//-----------------------------------------------------------------------------
// Name: ComputeLevelOfDetailTextures()
// Desc: Generate level-of-detail textures by compositing together alternating layers.
//-----------------------------------------------------------------------------
    ...
    for( UINT iLOD = 1; m_dwNumSlices >> iLOD; iLOD++ )
    {
        ...
        for( UINT iSliceDst = 0; iSliceDst < nSliceDst; iSliceDst++ )
        {
            ...
            for( UINT iMip = 0; iMip < nMip; iMip++ )
            {
                ...
                g_pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, vQuad, sizeof(QUADVERTEX) ); // one quad blends 2 or 3 textures
                pSurface->Release();
            }
        }
...
//-----------------------------------------------------------------------------
// Name: AlphaDivide()
// Desc: Divide alpha channel into color channel.  Leave alpha unchanged.
//-----------------------------------------------------------------------------
HRESULT AlphaDivide(LPDIRECT3DTEXTURE8 pTexture)
{
    ...
    for( UINT iMip = 0; iMip < nMip; iMip++ )
    {
        ...
        D3DLOCKED_RECT lockedRect;
        pTexture->LockRect( iMip, &lockedRect, NULL, 0l );
        UINT dwPixelSize = 4;
...
//-----------------------------------------------------------------------------
// Name: AlphaMultiply()
// Desc: Multiply alpha channel into color channel.  Leave alpha unchanged.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name: CompressNextTexture()
// Desc: Copy textures to new texture format
//-----------------------------------------------------------------------------
