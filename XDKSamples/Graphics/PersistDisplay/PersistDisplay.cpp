...
#define TRANSITION_FADEIN_END    2.0f    // Dolphin finishes fading-in 3 seconds
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( m_DolphinMesh1.Create( "Models\\Dolphin1.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    if( FAILED( m_DolphinMesh2.Create( "Models\\Dolphin2.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    if( FAILED( m_DolphinMesh3.Create( "Models\\Dolphin3.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    if( FAILED( m_SeaFloorMesh.Create( "Models\\SeaFloor.xbg" ) ) )
        return XBAPPERR_MEDIANOTFOUND;
    ...
    {
        // Get the persisted surface dimensions. This is important since the
        // values will be different if PAL vs. NTSC, etc.
        D3DSURFACE_DESC desc;
        pPersistedSurface->GetDesc( &desc );

        // Create an empty surface to hold the persistent screen
        m_pd3dDevice->CreateTexture( desc.Width, desc.Height, 1, 0, desc.Format,
                                     0, &m_pPersistedTexture );
        m_pPersistedTexture->GetSurfaceLevel( 0, &m_pPersistedSurface );
    
        // Copy persisted bits to our surface
        m_pd3dDevice->CopyRects( pPersistedSurface, NULL, 0, m_pPersistedSurface, NULL );
...
HRESULT CXBoxSample::RenderQuad( LPDIRECT3DTEXTURE8 pTexture, FLOAT fAlpha )
{
    ...
    v[1].p = D3DXVECTOR4( w-0.5f, 0-0.5f, 0, 0 );   v[1].tu = w;   v[1].tv = 0;
    v[2].p = D3DXVECTOR4( w-0.5f, h-0.5f, 0, 0 );   v[2].tu = w;   v[2].tv = h;
    v[3].p = D3DXVECTOR4( 0-0.5f, h-0.5f, 0, 0 );   v[3].tu = 0;   v[3].tv = h;
...
HRESULT CXBoxSample::Render()
{
    ...
    // we can have a transition effect. Render that effect here, after the
    ...
    // again, so it looks like the dolphin fades into existence.
    if( m_bDoingStartupTransition )
    {
    ...
    // Draw the title and frame rate
    if( !m_bDoingStartupTransition && !m_bAboutToLaunchNewTitle )
    {
        m_Font.Begin();
        m_Font.SetScaleFactors( 1.2f, 1.2f );
        m_Font.DrawText( 48, 36, 0xffffffff,  L"PersistDisplay" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        m_Font.DrawText(  64,  75, 0xffffffff, L"Press the " GLYPH_Y_BUTTON L" button to persist the display" );
        m_Font.DrawText(  64, 100, 0xffffffff, L"and relaunch this title" );
        m_Font.End();
    }
