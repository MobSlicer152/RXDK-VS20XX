...
//       10.25.02 - Added colorsign to filter the bumpmap
//
...
HRESULT CXBoxSample::InitBumpMap()
{
    ...
    // Create the bumpmap's texture object
    ...
    // Fill the newly created bumpmap
...
HRESULT CXBoxSample::Initialize()
{
    ...
    m_pBlockTexture     = m_xprResource.GetTexture( "Block" );
    m_pEarthTexture     = m_xprResource.GetTexture( "Earth" );
    m_pEarthBumpTexture = m_xprResource.GetTexture( "EarthBump" );
    m_pEnvMapTexture    = m_xprResource.GetTexture( "EarthEnvMap" );
...
HRESULT CXBoxSample::Render()
{
    ...
    // Generate sphere mapping coords for the environment map on stage 2
    D3DXMATRIX mat;
    ...
    // Filter the bumpmap takes some care, otherwise, adjacent values of 0x00
    // and 0xff could filter to some bogus intermediate value. So, make use of
    // the Xbox extension to treat the color components as signed values.
    m_pd3dDevice->SetTextureStageState( 1, D3DTSS_COLORSIGN, D3DTSIGN_RSIGNED|D3DTSIGN_GSIGNED|D3DTSIGN_BSIGNED );
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"BumpEarth" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 592, 38, 0xffffff00, m_strFrameRate, XBFONT_RIGHT );

        // Output description of what's being displayed
        m_Font.DrawText( 64,  80, 0xffffffff, L"Texture: " );
        m_Font.DrawText( 0xffffff00, m_bTextureOn ? L"ON" : L"OFF" );
        m_Font.DrawText( 64, 105, 0xffffffff, L"Bumpmap: " );
        m_Font.DrawText( 0xffffff00, m_bBumpMapOn ? L"ON" : L"OFF" );
