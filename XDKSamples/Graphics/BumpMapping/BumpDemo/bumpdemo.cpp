...
HRESULT CXBoxSample::Initialize()
{
    ...
    // Init vertex shader
    if( FAILED( XBUtil_CreateVertexShader( "Shaders\\BumpShader.xvu",
                                           dwShaderVertexDecl, &m_dwVertexShader ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    // Create some geometry
    m_Gears[0].Create( 0.45f, 3.0f, 15, 0.36f );
    m_Gears[1].Create( 0.60f,-3.0f, 15, 0.08f );
    m_Gears[2].Create( 0.90f, 1.5f, 30, 0.00f );
    m_Gears[3].Create( 0.90f,-0.5f, 30, 0.00f );
    m_Gears[4].Create( 0.30f, 1.5f, 10, 0.25f );
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    m_Gears[0].FrameMove( m_fAppTime );
    m_Gears[1].FrameMove( m_fAppTime );
    m_Gears[2].FrameMove( m_fAppTime );
    m_Gears[3].FrameMove( m_fAppTime );
    m_Gears[4].FrameMove( m_fAppTime );
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff,  L"BumpDemo" );
        ...
        m_Font.DrawText( 64, 100, 0xffffffff, L"Glossmap: " );
        m_Font.DrawText( 0xffffff00, m_bDrawGlossMap ? L"ON" : L"OFF" );
        m_Font.DrawText( 64, 125, 0xffffffff, L"Pass thru: " );
        m_Font.DrawText( 0xffffff00, m_bPassThru ? L"TRUE" : L"FALSE" );
        m_Font.End();
    }
