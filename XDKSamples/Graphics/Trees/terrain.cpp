...
#endif                
                // tri 0
                pIndices[iiTri]   = (WORD)(iVertex);
                pIndices[iiTri+1] = (WORD)(iVertex + m_width);
                pIndices[iiTri+2] = (WORD)(iVertex + m_width + 1);
                // tri 1
                pIndices[iiTri+3] = (WORD)(iVertex);
                pIndices[iiTri+4] = (WORD)(iVertex + m_width + 1);
                pIndices[iiTri+5] = (WORD)(iVertex + 1);
#if 0                
            }
            else
            {
                            ...
                            UINT width, UINT height)
{
    ...
    // Load the terrain vertex shader
    {
        ...
        m_dwVertexShader = 0;
        hr = XBUtil_CreateVertexShader("Shaders\\Terrain.xvu", vsdecl, &m_dwVertexShader);
        if(FAILED(hr) || !m_dwVertexShader)
        {
            OUTPUT_DEBUG_STRING("Terrain::Initialize : error loading Terrain.xvu\n");
...
HRESULT Terrain::RefineGrid(const D3DXVECTOR3 &vPosition, float fEpsilon)
{
    ...
    //float fRadius2 = fRadius*fRadius; // Used by L2 norm
...
HRESULT Terrain::DrawTerrain()
{
    ...
    D3DXMATRIX mat = matWorldNew * matView * matProjection;
    D3DXMATRIX matTranspose;
    D3DXMatrixTranspose(&matTranspose, &mat);

    // Vertex shader constant indices must match those in terrain.vsh
#define VSC_WORLD_VIEW_PROJECTION   80
#define VSC_DETAIL_TEXTURE_SCALE_OFFSET 84
#define VSC_SHADOW_TEXTURE_SCALE_OFFSET 85

    // Set vertex shader and constants
    g_pd3dDevice->SetVertexShader( m_dwVertexShader );
    g_pd3dDevice->SetVertexShaderConstant( VSC_WORLD_VIEW_PROJECTION, &matTranspose, 4);
    D3DXVECTOR4 detail_texgen(m_vTextureScale.x, m_vTextureScale.y, m_vTextureOffset.x, m_vTextureOffset.y);
    g_pd3dDevice->SetVertexShaderConstant( VSC_DETAIL_TEXTURE_SCALE_OFFSET, &detail_texgen, 1);
    D3DXVECTOR4 shadow_texgen(1.f, 1.f, 0.f, 0.f);
    g_pd3dDevice->SetVertexShaderConstant( VSC_SHADOW_TEXTURE_SCALE_OFFSET, &shadow_texgen, 1);
    ...
    g_pd3dDevice->SetTextureStageState( xx, D3DTSS_MAXANISOTROPY, 2 ); // make the terrain texture a little sharper as it recedes
