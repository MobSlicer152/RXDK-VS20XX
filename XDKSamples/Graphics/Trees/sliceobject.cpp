...
//
// Desc: CSliceObject represents a geometric object as textured slices that
...
HRESULT CSliceObject::Slice(UINT nDirection, SliceInfo *rSliceInfo) // slice in each of the directions
{
    ...
    for (m_iDirection = 0; m_iDirection < m_nDirection; m_iDirection++)
    {
        ...
        // Try loading from xpr files
        hr = m_rSliceTexture[m_iDirection].Load(Width, Height, nSlice, rSliceInfo[m_iDirection].strName);
        if (hr != S_OK)
        {
            // Generate the textures
            extern BOOL g_bCompressTextures;
#if _DEBUG
            extern BOOL g_bDebugExpand;
            g_bDebugExpand = FALSE;
            OUTPUT_DEBUG_STRING(" Slice...");
#endif
            hr = m_rSliceTexture[m_iDirection].GenerateSlices(
                Width, Height, nSlice, this, WrapU, WrapV,
                D3DFMT_A8R8G8B8); // See compressed format comment below.
            if (FAILED(hr))
                return hr;
            ...
            if (FAILED(hr))
                return hr;
            OUTPUT_DEBUG_STRING(" Saving...");
            m_rSliceTexture[m_iDirection].Save(rSliceInfo[m_iDirection].strName);
...
HRESULT CSliceObject::BeginDrawCubeSlices()
{
    ...
    DWORD xx = 0;
    static D3DTEXTUREALPHAKILL AlphaKill = D3DTALPHAKILL_ENABLE;
    g_pd3dDevice->SetTextureStageState(xx, D3DTSS_ALPHAKILL, AlphaKill);
