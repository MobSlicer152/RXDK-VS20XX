...
HRESULT CTree::Create(CHAR *strName, CXBPackedResource *pResource)
{
    ...
    // Load the tree vertex shader
    {
        ...
        m_dwVertexShader = 0;
        hr = XBUtil_CreateVertexShader("Shaders\\Tree.xvu", vsdecl, &m_dwVertexShader);
    ...
    if (!m_pMeshTrunk)
        return E_OUTOFMEMORY;
    if( FAILED( m_pMeshTrunk->Create( strTrunk, pResource )))
        return XBAPPERR_MEDIANOTFOUND;
...
HRESULT CTree::DrawTrunk(const D3DXVECTOR3 &vFrom)
{
    ...
    return m_pMeshTrunk->Render( dwFlags);
}
