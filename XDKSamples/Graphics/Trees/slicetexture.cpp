                                      ...
                                      D3DFORMAT fmt)
{
    ...
    for (i = 0; i < (int)nslices; i++)
    {
        ...
        extern D3DXVECTOR3 g_vDebugFromDir;
        D3DXVECTOR3 vSaveDebugFromDir = g_vDebugFromDir;
        g_vDebugFromDir = D3DXVECTOR3(3.f, 0.1f, 0.1f);
#endif
...
#if DEBUG_SHOW_SLICING
        g_vDebugFromDir = vSaveDebugFromDir;
        g_dwDebugPresentPause = 100; // delay after each slice
        DebugPresent();
        DebugClear();   // clear for next time
#endif
...
//        DebugTexture(pTexture, 0);
        ...
        if (fmt != D3DFMT_A8R8G8B8)
        {
            ...
            LPDIRECT3DTEXTURE8 pTextureSrc = pTexture;
            hr = CopyTexture(&pTextureDst, fmt, pTextureSrc);
            if (FAILED(hr))
                return hr;
            m_rpTexture[i] = pTextureDst; // already addref'd
...
#include "comp3.inl"
    ...
    for (UINT iLOD = 1; m_dwNumSlices >> iLOD; iLOD++)
    {
        ...
        {
            if (!apTextureDst[iSliceDst])
            {
                static DWORD nLevel = 0;
                // TODO: consider making the LOD textures smaller
                hr = g_pd3dDevice->CreateTexture(m_Width, m_Height, nLevel, 0, D3DFMT_A8R8G8B8, 0, &apTextureDst[iSliceDst]);
                if (FAILED(hr))
                    goto e_Exit;
            }
...
HRESULT CSliceTexture::SetTextureFormat(D3DFORMAT fmtNew)
{
    ...
    for (UINT iTexture = 0; iTexture < SLICETEXTURE_MAXSLICE * 2 - 1; iTexture++)   // convert all the slice textures
    {
        ...
        if (pTextureSrc == NULL)
            break;
#if DEBUG_SHOW_SLICING
        // Display the original slice texture
        extern HRESULT DebugTexture(LPDIRECT3DTEXTURE8 pTexture, UINT iLevel);
        extern DWORD g_dwDebugPresentPause;
        g_dwDebugPresentPause = 0; // delay after each slice
        DebugTexture(pTextureSrc, 0);
#endif      
        ...
        pTextureSrc->Release(); // we're done with the old texture
#if DEBUG_SHOW_SLICING
        // Display the compressed slice texture
        g_dwDebugPresentPause = 100; // delay after each slice
        DebugTexture(pTextureDst, 0);
#endif      
    }
    return S_OK;
}
...
//
//-----------------------------------------------------------------------------
HRESULT CSliceTexture::Load(UINT Width, UINT Height, UINT nSlice, CHAR *strName)
{
    return E_NOTIMPL;
}
...
//
//-----------------------------------------------------------------------------
HRESULT CSliceTexture::Save(CHAR *strName)
{
    return E_NOTIMPL;
}
