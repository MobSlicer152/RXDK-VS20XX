...
//
// Hist: 05.14.01 - Modified version to show off performance testing
//
...
class CXBoxSample : public CXBApplication
{
    ...
    LPDIRECT3DTEXTURE8       m_pLightTexture;  // Texture for flashlight lens flare
...
{
    // Create the fonts
    if( FAILED( m_Font16.Create( "Font.xpr" ) ) )
    ...
    if( FAILED( m_TerrainObject.Create( "Models\\StoneHenge.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    if( FAILED( m_FlashLightObject.Create( "Models\\Light.xbg", &m_xprResource ) ) )
        return XBAPPERR_MEDIANOTFOUND;
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    const D3DXVECTOR3 vLightLookat( g_vFlashLightPos + g_vFlashLightDir );
    const D3DXVECTOR3 vUp( 0.0f, 1.0f, 0.0f );
    D3DXMATRIX matLookAt;
    D3DXMatrixLookAtLH( &matLookAt, &g_vFlashLightPos, &vLightLookat, &vUp );
    ...
    if( FALSE == g_bPerfTestRunning )
    {
        ...
        else
        {
            ...
            if( g_bDisplayPerfTestMenu )
            {
                ...
                m_Font16.DrawText( 320,  50, 0xffffffff, L"Performance Test Options", XBFONT_CENTER_X );
...
#ifdef LTCG
                m_Font12.DrawText(  64,  75, 0xff00ffff, L"Performance tests are disabled in LTCG builds" );
#else
