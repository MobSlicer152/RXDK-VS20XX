...
XBHELP_CALLOUT g_HelpCallouts[] = 
{
    ...
    { XBHELP_A_BUTTON,     XBHELP_PLACEMENT_2, L"Home team\nscores" },
    { XBHELP_B_BUTTON,     XBHELP_PLACEMENT_2, L"Visitor\nscores" },
    { XBHELP_X_BUTTON,     XBHELP_PLACEMENT_2, L"Play food\nvendor" },
    { XBHELP_Y_BUTTON,     XBHELP_PLACEMENT_1, L"Play organ" },
    { XBHELP_BLACK_BUTTON, XBHELP_PLACEMENT_2, L"Announce\nscore" },
    { XBHELP_START_BUTTON, XBHELP_PLACEMENT_1, L"New game" },
...
//-----------------------------------------------------------------------------
void CXBoxSample::ReportScriptError( const CHAR* strMethod, const CHAR* strName,
                                     DMUS_SCRIPT_ERRORINFO* pErrorInfo )
{
#ifdef _DEBUG
...
HRESULT CXBoxSample::InitDirectMusic() 
{
    ...
    DirectMusicCreateInstance( CLSID_DirectMusicLoader, 
                               NULL, 
                               IID_IDirectMusicLoader8, 
                               (VOID**)&m_pLoader );

    // Create performance object
    DirectMusicCreateInstance( CLSID_DirectMusicPerformance, 
                               NULL,
...
HRESULT CXBoxSample::StartScript()
{
    ...
    if(! m_pMusicScript )
    {
        ...
        hr = m_pLoader->LoadObjectFromFile( CLSID_DirectMusicScript, 
...
//-----------------------------------------------------------------------------
// Name: SetVolume()
// Desc: Set volume on the main music audiopath
//-----------------------------------------------------------------------------
HRESULT CXBoxSample::SetVolume( LONG lVolume )
{
    ...
    lVolume = (lVolume - kInLow) * (kOutHigh - kOutLow) / (kInHigh - kInLow) + kOutLow;
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff, L"DMScript: DirectMusic AudioScript Sample\n" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );
        m_Font.DrawText( 0xffffffff, m_strScriptName );

        // Draw instructions
        DrawScore( 80, 100, GLYPH_A_BUTTON L" Add One to Home Team Score:", m_HomeTeamScore );
        DrawScore( 80, 130, GLYPH_B_BUTTON L" Add One to Visitors Score:", m_AwayTeamScore );
        m_Font.DrawText( 80, 160, 0xffffffff, GLYPH_X_BUTTON L" Trigger Food Vendor" );
        m_Font.DrawText( 80, 190, 0xffffffff, GLYPH_Y_BUTTON L" Trigger Organ Music" );
        m_Font.DrawText( 80, 220, 0xffffffff, GLYPH_BLACK_BUTTON L" Announce Score" );
        m_Font.DrawText( 58, 250, 0xffffffff, GLYPH_START1_BUTTON GLYPH_START2_BUTTON L"Start - Start New Game" );
        m_Font.DrawText( 80, 300, 0xffc0c0c0, L"The game is over after one team scores 10 runs." );
        m_Font.DrawText( 80, 400, 0xffffffff, L"Volume: " );
    
        // Draw the volume bar
        struct BACKGROUNDVERTEX { D3DXVECTOR4 p; D3DCOLOR color; };
        BACKGROUNDVERTEX v[4];
        FLOAT x1 = 170, x2 = x1 + (280*m_fVolume)/100;
        FLOAT y1 = 403, y2 = y1 + 20;
    ...
    XBSound_DrawLevelMeters( m_pDSound, 64.0f, 360.0f, 60.0f, 30.0f );
