...
//       audio and video
//
// Hist: 03.13.01 - New for April XDK
...
const DWORD NUM_SEGMENTS = sizeof(g_strSegments) / sizeof(g_strSegments[0]);
...
HRESULT CXBoxSample::Initialize()
{
    ...
    for( int i = 0; i < NUM_SEGMENTS; i++ )
    {
        m_pDMLoader->LoadObjectFromFile( CLSID_DirectMusicSegment, IID_IDirectMusicSegment8, 
                                         g_strSegments[i], (VOID**)&m_apMusic[i] );
    ...
    // Create a DirectMusicGraph, and tell the performance about it
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    if( m_DefaultGamepad.bPressedAnalogButtons[ XINPUT_GAMEPAD_B ] )
    {
        ...
        m_dwCurrent = ( m_dwCurrent + 1 ) % ( sizeof( g_strSegments ) / sizeof( g_strSegments[0] ) );
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff, L"DMTool" );
        m_Font.SetScaleFactors( 1.0f, 1.0f );

        swprintf( strBuffer, L"%S", g_strSegments[m_dwCurrent] );
        m_Font.DrawText( 64, 80, 0xffffffff, L"Playing: " );
