...
HRESULT CXBoxSample::LoadSounds()
{
    ...
    for( int i = 0; i < NUM_SOUNDS; i++ )
    {
        ...
        DWORD dwLoopStart  = 0;
        DWORD dwLoopLength = dwDuration;
        m_awfSounds[i].GetLoopRegion( &dwLoopStart, &dwLoopLength );

        // Set up values for the new buffer
        m_apBuffers[i]->SetBufferData( m_pbSampleData[i], dwDuration );
        m_apBuffers[i]->SetLoopRegion( dwLoopStart, dwLoopLength );
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( XAudioDownloadEffectsImage( "d:\\media\\dspimage.bin", 
                                            &EffectLoc, 
    ...
    dsbd.dwInputMixBin = DSMIXBIN_FXSEND_0;
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff, L"FXMultiPass" );
