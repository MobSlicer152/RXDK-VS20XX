...
public:
    ...
    virtual HRESULT RenderFrame( XBMESH_FRAME* pMesh, DWORD dwFlags );
    virtual HRESULT RenderMesh( XBMESH_DATA* pMesh, DWORD dwFlags );
...
public:
    ...
    virtual VOID RenderMeshCallback( DWORD dwFrame, XBMESH_FRAME* pFrame, DWORD dwFlags ) { }
    ...
    // rendering.
    virtual BOOL RenderCallback( DWORD dwSubset, XBMESH_SUBSET* pSubset, DWORD dwFlags ) { return TRUE; }
    
    // Render function. Call this function to render the hierarchy of frames
    // and meshes.
    HRESULT Render( DWORD dwFlags = 0x00000000 );

    // Function to find the radius of sphere centered at zero enclosing mesh.
    FLOAT ComputeRadius();
    
    // Find the bounding box of all the subsets
    HRESULT ComputeBoundingBox( D3DXVECTOR3* vMin, D3DXVECTOR3* vMax );
};
