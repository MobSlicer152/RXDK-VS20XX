...
//-----------------------------------------------------------------------------
VOID XBUtil_DrawRect( FLOAT x1, FLOAT y1, FLOAT x2, FLOAT y2,
                                   ...
                                   DWORD* pdwVertexShader )
{
    ...
    hr = D3DDevice::CreateVertexShader( pdwVertexDecl, (const DWORD*)pData,
                                        pdwVertexShader, 0 );
...
HRESULT XBUtil_CreatePixelShader( const CHAR* strFilename, DWORD* pdwPixelShader )
{
    ...
    if( FAILED( hr = D3DDevice::CreatePixelShader( &(psdf.Psd), pdwPixelShader ) ) )
    {
        CloseHandle( hFile );
                                           ...
                                           LPDIRECT3DCUBETEXTURE8* ppCubeMap )
{
    ...
    // Create the cube map
    if( FAILED( hr = D3DDevice::CreateCubeTexture( dwSize, 1, 0, D3DFMT_X8R8G8B8, 
...
D3DXVECTOR3 XBUtil_EvaluateHermite( const D3DXVECTOR3& p0, const D3DXVECTOR3& p1, 
                                    ...
                                    FLOAT u )
{
    // Generate coefficients from the two end points and two tangents
...
//-----------------------------------------------------------------------------
VOID XBUtil_GetSplinePoint( const D3DXVECTOR3* pSpline, DWORD dwNumSplinePts,
                            FLOAT t, D3DXVECTOR3* pvPoint, D3DXVECTOR3* pvTangent )
{
    DWORD p0 = ( t > 1.0 ) ? (DWORD)floorf(t)-1 : dwNumSplinePts-1;
    DWORD p1 = ( p0 < dwNumSplinePts-1 ) ? p0 + 1 : 0;
    DWORD p2 = ( p1 < dwNumSplinePts-1 ) ? p1 + 1 : 0;
    DWORD p3 = ( p2 < dwNumSplinePts-1 ) ? p2 + 1 : 0;
    FLOAT u  = t - floorf(t);
                          ...
                          DWORD dwColor, BOOL bRenderAxes )
{
    ...
    for( FLOAT u = 0; u < dwNumSplinePts; u += 1.0f )
    {
        ...
        D3DDevice::SetRenderState( D3DRS_TEXTUREFACTOR, dwColor );
        D3DDevice::DrawPrimitiveUP( D3DPT_LINELIST, 1, p, sizeof(D3DXVECTOR3) );

        if( bRenderAxes )
        {
            p[1] = p[0] + vTangent/4;
            D3DDevice::SetRenderState( D3DRS_TEXTUREFACTOR, 0xffff0000 );
            D3DDevice::DrawPrimitiveUP( D3DPT_LINELIST, 1, p, sizeof(D3DXVECTOR3) );

            p[1] = p[0] + vSide/4;
            D3DDevice::SetRenderState( D3DRS_TEXTUREFACTOR, 0xff00ff00 );
            D3DDevice::DrawPrimitiveUP( D3DPT_LINELIST, 1, p, sizeof(D3DXVECTOR3) );
                                  ...
                                  DWORD Declaration[MAX_FVF_DECL_SIZE] )
{
    ...
    for( DWORD i=0; i<dwNumTextures; i++ )
    {
        ...
        if( lTexCoordSize == D3DFVF_TEXCOORDSIZE1(i) ) dwNumTexCoords = D3DVSDT_FLOAT1;
        if( lTexCoordSize == D3DFVF_TEXCOORDSIZE2(i) ) dwNumTexCoords = D3DVSDT_FLOAT2;
        if( lTexCoordSize == D3DFVF_TEXCOORDSIZE3(i) ) dwNumTexCoords = D3DVSDT_FLOAT3;
        if( lTexCoordSize == D3DFVF_TEXCOORDSIZE4(i) ) dwNumTexCoords = D3DVSDT_FLOAT4;
...
//-----------------------------------------------------------------------------
HRESULT XBUtil_LoadFile( const CHAR* strFile, VOID** ppFileData, DWORD* pdwSize )
{
    ...
    // The file had better fit into memory
    assert( dwFileSize <= 64 * 1024 * 1024 );
