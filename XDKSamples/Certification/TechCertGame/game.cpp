...
// Desc: Update the vehicles positions and wheels.
//-----------------------------------------------------------------------------
...
// Desc: Constructor
//-----------------------------------------------------------------------------
Game::Game( CXBFont* pFont, AudioEngine& audioengine, CSoundEffect& sndeffect )
:
...
//-----------------------------------------------------------------------------
VOID Game::Start( BOOL bRecordDemo, BOOL bPlayDemo, BOOL bIsVibrationOn, FLOAT fMusicVolume, FLOAT fEffectVolume, UINT uSelectedSoundtrack, const CHAR* strDemoName )
{
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        SAFE_RELEASE( m_pScreenShotTexture );
        hr = g_pd3dDevice->CreateTexture( SCREEN_SHOT_SIZE, SCREEN_SHOT_SIZE,
                                          1, 0, Desc.Format,
                                          D3DPOOL(), &m_pScreenShotTexture );
        assert( SUCCEEDED(hr) );
    }

    // Start a song playing from a randomly selected soundtrack
    m_AudioEngine.SelectSoundtrack( uSelectedSoundtrack );
    m_AudioEngine.SetRandom( TRUE );
    m_AudioEngine.SetVolume( XACT_CATEGORY_BGMUSIC, m_fMusicVolume );
    m_AudioEngine.SetVolume( XACT_CATEGORY_SFX, m_fEffectVolume );
    m_AudioEngine.Play( XACT_SOUNDBANK_GAME_MUSIC );
                         ...
                         FLOAT fElapsedTime )
{
    ...
    if( !m_bIsPaused )
    {
        ...
        // Play 'honk' sound effect - check for m_iNumActors to make
        // sure the game was actually started (game is run during
        // load screen, even if entered from start menu).
        if( pGamepad->bPressedAnalogButtons[ XINPUT_GAMEPAD_A ] &&
            m_iNumActors > 0 )
        {
            m_AudioEngine.Play( XACT_SOUNDBANK_GAME_HONK );
        }
    }
    ...
    FLOAT fForce     = 0.0f;
    FLOAT fVibration = 0.0f;

    if( !m_bIsPaused )
    {
        // Run physics
        fForce = m_pPhysicsSystem->Simulate( fTime - fElapsedTime, fTime );
        fVibration = fForce;

        // Clamp vibration from force
        if( fVibration < 0.0f )
            fVibration = 0.0f;
        else if( fVibration > 1.0f )
            fVibration = 1.0f;

        // Do controller vibration (no more than once every tenth second)
        if( m_bIsVibrationOn && m_VibrationTimer.GetElapsedSeconds() > 0.1f )
        {
        ...
        // Play crash sound effects where appropriate
        if( fForce >= 4000.0f && !m_fInCrash )
        {
            m_fInCrash = TRUE;
            m_AudioEngine.Play( XACT_SOUNDBANK_GAME_CRASH );
        }

        // Reset crash sound effect
        if( fForce <= 1000.0f && m_fInCrash )
        {
            m_fInCrash = FALSE;
        }
    }
...
*/
    ...
    m_pFont->DrawText( 64, 50, 0xffffff00, STRING(GAME_NAME) );

    // Show frame rate in debug mode
#if _DEBUG
    if( strFrameRate != NULL )
        m_pFont->DrawText( 450, 50, 0xffffff00, strFrameRate );
#else
    (VOID)strFrameRate;
#endif
...
VOID Game::ShowLoadingScreen()
{
    ...
    // Show loading text
    m_pFont->DrawText( 320, 220, 0xFFFFFFFF, STRING(LOADINGX), 
                       XBFONT_CENTER_X );
...
{
    // Only play/pause sound if
    // the game is currently running
    if( m_iNumActors )
    {
        if( !m_bIsPaused && bPaused )
        {
            // We should pause audio only once on the transition
            // from playing to paused
            m_AudioEngine.Pause();
            m_SoundEffect.Pause();
        }
        else if( m_bIsPaused && !bPaused )
        {
            // Unpause audio
    ...
    if( m_bIsPaused )
    {
        assert( m_pScreenShotTexture != NULL );

        LPDIRECT3DSURFACE8 pSurfScreenShot;
        HRESULT hr = m_pScreenShotTexture->GetSurfaceLevel( 0, &pSurfScreenShot );
        assert( SUCCEEDED(hr) );
...
{
    // TCR Saved Game Roaming
    // None of the data within the saved game references other content

    // In the future we could save the world objects here
                             ...
                             D3DCOLOR color, FLOAT radius, FLOAT mass )
{
    ...
    {
        // Compute inertia tensor.
        FLOAT coeff = mass * 1.0f / 8.0f * D3DX_PI * radius * radius;
        ...
        /*
        float fDensity = mass / (4.0f / 3.0f * D3DX_PI * radius * radius * radius);
        float fMass;
        Vector3 v3CoM;
        Matrix3 m3IT;
...
VOID Game::CheckForFreeSpace() const
{
    ...
    if( qwFreeBytes < ULONGLONG(dwBytesRequired) )
    {
        ...
        DWORD dwBlocksNeeded = ( dwBytesRequired - DWORD(qwFreeBytes) )/
                               CXBStorageDevice::GetBlockSize();

        DWORD dwTotalBlocks = dwBytesRequired / CXBStorageDevice::GetBlockSize();

        // Show text
        WCHAR strFree[ 256 ];
        wsprintfW( strFree, STRING(FREE), dwBlocksNeeded );
        m_pFont->DrawText( 320, 160, 0xFFFFFFFF, strFree, XBFONT_CENTER_X );
        ...
        for( ;; )
        {
            ...
            if( pGamepad != NULL )
            {
                ...
                if( pGamepad->bPressedAnalogButtons[ XINPUT_GAMEPAD_B ] )
                {
                    ...
                    LaunchDash.dwParameter2 = dwTotalBlocks;
