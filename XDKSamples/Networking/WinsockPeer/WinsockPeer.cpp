...
//       05.06.03 - Updated for June 2003 XDK release
//
...
const DWORD MAX_PLAYERS = 4;            // Max players
...
class CXBoxSample : public CXBApplication
{
    ...
    BOOL         m_bNetGood;             // TRUE if link status good
...
HRESULT CXBoxSample::FrameMove()
{
        ...
        m_LinkStatusTimer.GetElapsedSeconds() > CHECK_LINK_STATUS )
    {
        ...
        m_bNetGood = ( dwStatus & XNET_ETHERNET_LINK_ACTIVE ) != 0;
    }
            ...
            switch( m_CurrItem )
            {
                ...
                {    
                    ...
                    for( DWORD i = 0; i < MAX_GAME_NAMES; ++i )
                    {
                        ...
                        XBRandName_GetRandomName( strGameName, MAX_GAME_NAME );
...
VOID CXBoxSample::FrameMoveGame( Event ev )
{
    ...
    // Handle requests from players with older/newer versions of the game
    if( ProcessVersionMismatch() )
        return;
...
{
    RenderHeader();
    RenderFooter( GLYPH_A_BUTTON L"Select", GLYPH_WHITE_BUTTON L"Help" );
    ...
    {
        DWORD dwColor = ( m_CurrItem == i && m_bNetGood ) ? COLOR_HIGHLIGHT : 
                                                            COLOR_NORMAL;
    ...
    if( m_bNetGood )
        m_Font.DrawText( 260.0f, fYtop + (fYdelta * m_CurrItem ), 0xff00ff00,
                         GLYPH_RIGHT_TICK, XBFONT_RIGHT );

    m_Font.DrawText( 320, 400, COLOR_NORMAL, m_bNetGood ? 
...
VOID CXBoxSample::RenderGame()
{
    ...
    // Game name
    m_Font.DrawText( 220, 120, COLOR_NORMAL, L"Game name: " );
    m_Font.DrawText( COLOR_GREEN, m_strGameName );

    // Player name
    m_Font.DrawText( 220, 144, COLOR_NORMAL, L"Your name: " );
    m_Font.DrawText( COLOR_GREEN, m_strPlayerName );

    // Number of players
    WCHAR strBuffer[80];
    wsprintfW( strBuffer, L"%lu", m_Players.size() + 1 );
    m_Font.DrawText( 220, 168, COLOR_NORMAL, L"Players in game: " );
    m_Font.DrawText( COLOR_GREEN, strBuffer );

    // Current status
    m_Font.DrawText( 220, 192, COLOR_NORMAL, L"Status: " );
    m_Font.DrawText( COLOR_GREEN, m_strStatus );
...
VOID CXBoxSample::RenderSelectName()
{
    ...
    RenderHeader();
    RenderFooter( GLYPH_A_BUTTON L"Select   " GLYPH_B_BUTTON L"Cancel", GLYPH_WHITE_BUTTON L"Help" );

    m_Font.DrawText( 320, 110, 0xffffffff, L"Select a game name", XBFONT_CENTER_X );
    ...
    // Show selected item with little triangle
    m_Font.DrawText( 280.0f, fYtop + (fYdelta * m_CurrItem ), 0xff00ff00,
                     GLYPH_RIGHT_TICK, XBFONT_RIGHT );
}
...
{
    RenderHeader();
    RenderFooter( GLYPH_B_BUTTON L"Cancel", NULL );

    m_Font.DrawText( 320, 240, 0xffffffff, L"Searching For Active Games", XBFONT_CENTER_X | XBFONT_CENTER_Y );
}
...
VOID CXBoxSample::RenderSelectGame()
{
    ...
    m_Font.DrawText( 320, 110, 0xffffffff, L"Select game to join", XBFONT_CENTER_X );
    ...
    // Show selected item with little triangle
    m_Font.DrawText( 140.0f, fYtop + (fYdelta * m_CurrItem ), 0xff00ff00,
                     GLYPH_RIGHT_TICK, XBFONT_RIGHT );
}
...
{
    RenderHeader();
    RenderFooter( GLYPH_A_BUTTON L"Continue", NULL );
...
VOID CXBoxSample::RenderHeader()
{
    ...
    m_Font.DrawText( 48, 36, 0xffffffff,  L"WinsockPeer" );
    if( m_bIsHost )
        m_Font.DrawText( 0xffffffff,  L" (host)" );
...
VOID CXBoxSample::RenderFooter( WCHAR* strLeftText, WCHAR* strRightText )
{
    ...
    if( strRightText )
        m_Font.DrawText( 640-48, 480-36-24, 0xffffffff, strRightText, XBFONT_RIGHT );
}
...
VOID CXBoxSample::Clear()
{
    ...
    *m_strHostName   = 0;
    ...
    XBRandName_GetRandomName( m_strPlayerName, MAX_PLAYER_NAME );
}
...
CXBoxSample::InitStatus CXBoxSample::InitXNet()
{
    ...
    while( !m_bHaveLocalAddress )
    {
        ...
        if( dwStatus != XNET_GET_XNADDR_PENDING )
            m_bHaveLocalAddress = TRUE;
    }

    // Clear old BROADCAST_VERSION bits in preparation for sending and
    // receiving new broadcast packets
...
VOID CXBoxSample::ProcessWave( const SOCKADDR_IN& saFrom )
{
    ...
    if( i != m_Players.end() )
    {
        wsprintfW( m_strStatus, L"%.*s waved", MAX_PLAYER_NAME, i->strPlayerName );
    }
}
...
//-----------------------------------------------------------------------------
// Name: ProcessVersionMismatch()
// Desc: Determine if other versions have attempted to find this game
//-----------------------------------------------------------------------------
BOOL CXBoxSample::ProcessVersionMismatch()
{
    // If we received broadcast requests from other incompatible versions
    // of this XBE, notify the player. This can happen if the two 
    // XBEs had the same title IDs, but different LAN keys and version
    // numbers. This scenario can occur when Live titles receive Auto-updates.
    DWORD dwStatus = XNetGetBroadcastVersionStatus( TRUE );
    if( dwStatus != 0 )
    {
        // We received one or more broadcast packets from an incompatible
        // version. The packets were discarded, but the status bits
        // indicate that another WinsockPeer is active.
        wsprintfW( m_strStatus, L"A%.*s version\n"
                                L"failed to join", 
                                8, dwStatus & XNET_BROADCAST_VERSION_OLDER ?
                                L"n older" : L" newer" );
        return TRUE;
    }               
    return FALSE;                 
