...
//       08.20.02 - Updated for Sep release
//
...
HRESULT CXBoxSample::Initialize()
{
    ...
    if( FAILED( XBOnline_GetUserList( m_UserAccountList, &m_dwNumUsers ) ) )
        return E_FAIL;
...
VOID CXBoxSample::Reset()
{
    ...
    for( DWORD i = 0; i < XONLINE_MAX_LOGON_USERS; ++i )
    {
        ...
        m_Users[i].dwLaunchReason = XLD_LAUNCH_DASHBOARD_MAIN_MENU; 
    }
}
...
HRESULT CXBoxSample::FrameMove()
{
    ...
    if( m_bSignedOn )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            {
                wcscpy( m_strMessage, L"You have been signed out because a\n"
                                      L"duplicate account signed in on another Xbox" );
            }
            else
            {
                wcscpy( m_strMessage, L"Connection was lost. Must sign in again." );
            }
            m_State = STATE_ERROR;
        }
        else
...
HRESULT CXBoxSample::Render()
{
    ...
    else
    {
        ...
        m_Font.DrawText( 48, 36, 0xffffffff, L"Auth" );
        ...
        switch( m_State )
        {
                    ...
                    switch( m_Users[i].State )
                    {
                        ...
                        case STATE_USER_SELECT_ACCOUNT:
                            m_UI.RenderUserSelectAccount( i, 
                                ...
                                m_Users[i].dwTopItem,
                                m_UserAccountList, m_dwNumUsers );
                            break;
                        case STATE_USER_CONFIRM_SPONSOR:
            ...
            case STATE_SIGNING_ON:
                ...
                break;
            case STATE_ERROR:
                m_UI.RenderMessage( m_strMessage, m_bAllowBootToDash );
                break;
            default:
                assert( FALSE );
                break;
        ...
        if( m_State != STATE_SIGNING_ON && !IsUserInPinEntry() )
            m_Font.DrawText( 360, 420, 0xffffffff, GLYPH_WHITE_BUTTON L"Help" );    
    }
...
VOID CXBoxSample::UpdateUserStatePINEntry( CUser& User, Event ev )
{
    ...
    switch( ev )
    {
        case EV_BUTTON_A:
            // Check PIN
            User.dwCurrItem = 0;
            if( 0 == memcmp( User.Account.passcode, User.Passcode, XONLINE_PASSCODE_LENGTH ) )
            {
...
VOID CXBoxSample::UpdateStateCreateAccount( Event ev )
{
    switch( ev )
    {
        ...
        default:
            ...
            {
                m_dwNumUsers = 0;
                XBOnline_GetUserList( m_UserAccountList, &m_dwNumUsers );
                if( m_dwNumUsers > 0 )
                {
                    m_State = STATE_USER_EVENTS;
                }
    ...
    switch( ev )
    {
        case EV_BUTTON_A:
        {
            ...
            else
            {
                ...
                swprintf( User.strName, L"%S", m_UserAccountList[User.dwCurrItem].szGamertag );

                // Check if a passcode is required.  Note that
                // passcodes are strictly for *client* side
                // authentication
                if( m_UserAccountList[User.dwCurrItem].dwUserOptions & XONLINE_USER_OPTION_REQUIRE_PASSCODE )
                {
                    User.State = STATE_USER_PIN_ENTRY;
        ...
        case EV_DOWN:
            ...
            {
                if( User.dwTopItem + MAX_ACCOUNTS_DISPLAYED < m_dwNumUsers )
                    ++User.dwTopItem;
            }
            
            // Move to next item
            if( User.dwCurrItem < m_dwNumUsers - 1 )
                ++User.dwCurrItem;
            break;
            ...
            if( m_Users[i].State != STATE_USER_PRE_SIGN_ON )
            {
                ...
                if( SUCCEEDED( hr ) )
                {
                    if( !m_Users[i].bGuest )
                    {
                        ...
                        m_Users[i].bVoice = XOnlineIsUserVoiceAllowed( m_Users[i].Account.xuid.dwUserFlags ) && 
...
VOID CXBoxSample::HandleSignOnError( HRESULT hr )
{
    ...
    switch( hr )
    {
        ...
        case XONLINE_E_LOGON_UPDATE_REQUIRED:
            ...
            swprintf( m_strMessage, L"Title update failed with error 0x%x", hrTitleUpdate );
            break;
        case XONLINE_E_LOGON_SERVERS_TOO_BUSY:
            swprintf( m_strMessage, L"The Xbox Live Sign In servers are too busy. "
                                    L"Try again later." );
            break;
        default:
            m_bAllowBootToDash = TRUE;      
...
VOID CXBoxSample::HandleServiceError( HRESULT hr, DWORD dwServiceId )
{
    ...
    switch( hr )
    {
        case XONLINE_E_LOGON_SERVICE_NOT_AUTHORIZED:
            swprintf( m_strMessage, L"Access to service %d is denied", dwServiceId );
            break;

        case XONLINE_E_LOGON_SERVICE_TEMPORARILY_UNAVAILABLE:
            swprintf( m_strMessage, L"Service %d is unavailable", dwServiceId );
            break;

        default:
            swprintf( m_strMessage, L"Error 0x%x logging into service %d", hr, dwServiceId );
    }
}
                                           ...
                                           const WCHAR* strFormat, ... )
{
    ...
    INT iChars = wvsprintfW( User.strError, strFormat, pArgList );
    assert( iChars < 1024 );
