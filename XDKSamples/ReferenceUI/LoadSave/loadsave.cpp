...
// Maximum number of blocks ever displayed for cert requirements
...
const FLOAT INITIAL_REPEAT = 0.333f; // 333 mS
const FLOAT STD_REPEAT     = 0.040f; // 40 mS

// Used bar color values
const FLOAT BAR_FULL = 0.95f;
const FLOAT BAR_WARN = 0.85f;

// Maximum message box time for status messages
const FLOAT STATUS_SECONDS = 0.5f;

// Minimum message box time for device activity messages
const FLOAT DEVICE_SECONDS    = 3.0f;
const FLOAT HD_DEVICE_SECONDS = 1.0f;
...
const D3DCOLOR MB_TEXT_COLOR     = 0xffffffff; // White
    ...
    *strFreeBlocks = 0;
    ...
    if( GetSize( qwTotalBytes, qwUsedBytes, qwFreeBytes ) )
    {
        ...
        if( dwTotalBlocks > MAX_BLOCKS )
            wcscpy( strTotalBlocks, strMAX_BLOCKS[ XGetLanguage() ] );
        else
            CXBConfig::FormatInt( dwTotalBlocks, strTotalBlocks );

        if( dwFreeBlocks > MAX_BLOCKS )            
            wcscpy( strFreeBlocks, strMAX_BLOCKS[ XGetLanguage() ] );
        else
                                      ...
                                      const CXBLoadSave* pContext ) const
{
    ...
    // MU or Xbox image
    pContext->RenderTile( rcImage, ( dwState != 0 ) ? pTextureBad : pTextureGood, 0xffffffff );

    // Used bar
    DWORD dwColor = BARCOLOR_NORMAL;
    if( fPercentUsed > BAR_FULL )
        dwColor = BARCOLOR_FULL;
    else if( fPercentUsed > BAR_WARN )
...
HRESULT CXBLoadSave::Initialize()
{
    ...
    // Create the fonts
    if( FAILED( m_FontBig.Create( "Font16.xpr" ) ) ||
        FAILED( m_FontMed.Create( "Font12.xpr" ) ) ||
        FAILED( m_FontSmall.Create( "Font9.xpr" ) )  )
    ...
    // Load our textures
    if( FAILED( m_xprResource.Create( "LoadSaveResource.xpr" ) ) )
        return XBAPPERR_MEDIANOTFOUND;

    // Load our textures from the bundled resource
    m_ptXbox       = m_xprResource.GetTexture( "Xbox" );
    m_ptXboxSel    = m_xprResource.GetTexture( "Xbox_Sel" );
    m_ptMU         = m_xprResource.GetTexture( "MU" );
    m_ptWideMU     = m_xprResource.GetTexture( "MUWide" );
    m_ptMUSel      = m_xprResource.GetTexture( "MU_Sel" );
    m_ptBackground = m_xprResource.GetTexture( "Background" );
    m_ptMsgBox     = m_xprResource.GetTexture( "MsgBox" );
    m_ptPlainBack  = m_xprResource.GetTexture( "PlainBack" );
    m_ptUp         = m_xprResource.GetTexture( "Up" );
    m_ptDown       = m_xprResource.GetTexture( "Down" );
    m_ptMUBad      = m_xprResource.GetTexture( "MU_Bad" );
    ...
    m_pd3dDevice->CreateVertexBuffer( 4 * sizeof(CUSTOMVERTEX), 0, 0, 0, &m_pvbBackground );
...
BOOL CXBLoadSave::SaveGame()
{
    ...
    DWORD dwCreate = SavedGame.CreateGame( chDestDrive, strGameName );
    if( dwCreate != ERROR_SUCCESS )
    {
        ...
        {
            bCurrGameGone = m_GameList[ m_iCurrGame ].DeleteGame();
            dwCreate = SavedGame.CreateGame( chDestDrive, strGameName );
        }
...
BOOL CXBLoadSave::SaveGame( const CHAR* strFile )
{
    ...
    DWORD dwSuccess = XCalculateSignatureEnd( hSignature, pSignature );
    assert( dwSuccess == ERROR_SUCCESS );
    (VOID)dwSuccess; // avoid compiler warning
...
BOOL CXBLoadSave::LoadGame()
{
    ...
    DWORD dwSuccess = XCalculateSignatureEnd( hSignature, pCalcSignature );
    assert( dwSuccess == ERROR_SUCCESS );
    (VOID)dwSuccess; // avoid compiler warning
...
DWORD CXBLoadSave::GetMuState( DWORD dwError ) // static
{
    ...
    switch( dwError )
    {
        ...
        // Any other error indicates the MU is probably unusable
    ...
    switch( ev )
    {
        ...
        case EV_START_BUTTON:
            if( m_Mode == MODE_LOAD )
            {
                ...
                StartMsgBox( strLoad, GAME_LOAD, DEVICE_SECONDS );
            }
            else // MODE_SAVE
            {
...
VOID CXBLoadSave::StartMsgBoxSave()
{
    ...
    if( m_iCurrDev == 0 )
    {
        ...
        fDeviceSeconds = HD_DEVICE_SECONDS;
    }
    else // MU
    {
        WCHAR strName[ MAX_DEVNAME ];
...
VOID CXBLoadSave::RenderStartScreen() const
{
    ...
    m_FontMed.DrawText( 320.0f, 80.0f, 0xffffffff,
    ...
    {
        m_FontMed.DrawText( 320.0f, 350.0f, 0xffffffff,
                            L"Localized terminology not final.\n"
                            L"See Xbox Guide for final terms.",
                            XBFONT_CENTER_X );
    }

    m_FontBig.DrawText( 280.0f, 400.0f, 0xffffffff, GLYPH_A_BUTTON );
    m_FontBig.DrawText( 0xffffffff, L"Continue" );
}
...
{
    // Clear the backbuffer
    m_pd3dDevice->Clear( 0L, NULL, D3DCLEAR_TARGET, 0x00808080, 1.0f, 0L );
    ...
    m_FontSmall.DrawText( 320.0f, 320.0f, 0xffffff00, strILLUS_GRAPHICS[ m_iLang ],
                          XBFONT_CENTER_X );

    DrawButton( 280.0f, GLYPH_A_BUTTON );
}
...
VOID CXBLoadSave::RenderDevice() const
{
    ...
    DrawButton(  80.0f, GLYPH_A_BUTTON );
    DrawButton( 460.0f, GLYPH_B_BUTTON );
}
...
VOID CXBLoadSave::RenderGameList() const
{
    ...
    // Device image
    const StorageDev& CurrDev = m_DeviceList[ m_iCurrDev ];
    RenderTile( g_avUIRects[GS_DEVICEIMG], CurrDev.pWideTexture, 0xffffffff );
    ...
    {
        RenderTile( g_avUIRects[GS_METAIMG], pGameImage, 0xffffffff );
        pGameImage->Release();
    }
    ...
    if( bShowTopArrow )
        RenderTile( g_avUIRects[GS_UP], m_ptUp, 0xffffffff );
    if( bShowBtmArrow )
        RenderTile( g_avUIRects[GS_DOWN], m_ptDown, 0xffffffff );

    // Buttons
    DrawButton(  80.0f, GLYPH_A_BUTTON );
    DrawButton( 270.0f, GLYPH_Y_BUTTON );
    DrawButton( 460.0f, GLYPH_B_BUTTON );
}
...
//-----------------------------------------------------------------------------
VOID CXBLoadSave::DrawButton( FLOAT fX, const WCHAR* strButton ) const
{
    m_FontBig.DrawText( fX, 400.0f, 0xffffffff, strButton );

    if( !_wcsicmp( strButton, GLYPH_A_BUTTON ) )
        m_FontBig.DrawText( 0xffffffff, strA_SELECT[m_iLang] );
    if( !_wcsicmp( strButton, GLYPH_B_BUTTON ) )
        m_FontBig.DrawText( 0xffffffff, strB_BACK[m_iLang] );
    if( !_wcsicmp( strButton, GLYPH_Y_BUTTON ) )
        m_FontBig.DrawText( 0xffffffff, strY_DELETE[m_iLang] );
}
                              ...
                              CXBFont& Font, DWORD dwFlags ) const
{
    ...
    RenderTile( rc, m_ptMsgBox, 0xffffffff );
...
VOID CXBLoadSave::RenderTile( const D3DXVECTOR4& rc, 
                              const LPDIRECT3DTEXTURE8 pTile, DWORD dwColor,
                              BOOL bPulse ) const
{
    if( pTile == NULL )
        return;
    ...
    if( bPulse )
    {
        // If we're pulsing, adjust the alpha
        dwAlpha = (DWORD)( 255 * ( cosf( 4.0f * m_fTime ) + 1.0f ) / 3.0f + 1.0f / 3.0f );
    }
    ...
    m_pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, (dwAlpha<<24) | (dwColor&0x00ffffff) );
    m_pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE );
    ...
    m_pd3dDevice->SetVertexData4f( D3DVSDE_VERTEX, rc.x - 0.5f, rc.y - 0.5f, 1.0f, 1.0f );
    m_pd3dDevice->SetVertexData2f( D3DVSDE_TEXCOORD0, 1.0f, 0.0f );
    m_pd3dDevice->SetVertexData4f( D3DVSDE_VERTEX, rc.z - 0.5f, rc.y - 0.5f, 1.0f, 1.0f );
    m_pd3dDevice->SetVertexData2f( D3DVSDE_TEXCOORD0, 1.0f, 1.0f );
    m_pd3dDevice->SetVertexData4f( D3DVSDE_VERTEX, rc.z - 0.5f, rc.w - 0.5f, 1.0f, 1.0f );
    m_pd3dDevice->SetVertexData2f( D3DVSDE_TEXCOORD0, 0.0f, 1.0f );
    m_pd3dDevice->SetVertexData4f( D3DVSDE_VERTEX, rc.x - 0.5f, rc.w - 0.5f, 1.0f, 1.0f );
    m_pd3dDevice->End();
                             ...
                             DWORD dwColor ) const
{
    ...
    D3DXVECTOR4 vBarVertices[4];
    vBarVertices[0] = D3DXVECTOR4( vecBar.x - 0.5f, vecBar.y - 0.5f, 1.0f, 1.0f );
    vBarVertices[1] = D3DXVECTOR4( vecBar.x + fPercent*(vecBar.z - vecBar.x) - 0.5f,
                                   vecBar.y - 0.5f, 1.0f, 1.0f );
    vBarVertices[2] = D3DXVECTOR4( vecBar.x + fPercent*(vecBar.z - vecBar.x) - 0.5f,
                                   vecBar.w - 0.5f, 1.0f, 1.0f );
    vBarVertices[3] = D3DXVECTOR4( vecBar.x - 0.5f, vecBar.w - 0.5f, 1.0f, 1.0f );

    
    // Line-strip rectangle for border
    D3DXVECTOR4 vBorderVertices[4];
    vBorderVertices[0] = D3DXVECTOR4( vecBar.x, vecBar.w, 1.0f, 1.0f );
    vBorderVertices[1] = D3DXVECTOR4( vecBar.x, vecBar.y, 1.0f, 1.0f );
    vBorderVertices[2] = D3DXVECTOR4( vecBar.z, vecBar.y, 1.0f, 1.0f );
    vBorderVertices[3] = D3DXVECTOR4( vecBar.z, vecBar.w, 1.0f, 1.0f );
    ...
    m_pd3dDevice->DrawVerticesUP( D3DPT_QUADLIST, 4, vBarVertices, sizeof(D3DXVECTOR4) );

    // Then render the linestrip border in BARBORDER
    m_pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, BARBORDER );
    m_pd3dDevice->DrawVerticesUP( D3DPT_LINELOOP, 4, vBorderVertices, sizeof(D3DXVECTOR4) );
}
...
VOID CXBLoadSave::RenderSelection( const D3DXVECTOR4& rc, FLOAT fOffset ) const
{
    ...
    RenderTile( vec, m_ptXboxSel, 0xffffffff, TRUE );
}
...
DWORD CXBLoadSave::GetSavedGameSize() const
{
    ...
    DWORD dwSaveGameSize = CurrDev.GetFileBytes( m_dwSavedGameBytes + dwHeader );
